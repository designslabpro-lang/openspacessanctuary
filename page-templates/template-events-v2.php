...
<section class="oss-section oss-section--cream">
	<div class="oss-container">
		...
		<?php else : ?>
			<div class="oss2-filters" data-oss-filter-bar="oss2-events-list" role="group" aria-label="<?php esc_attr_e( 'Filter events by type', 'astra-child' ); ?>">
				<button type="button" class="oss2-filters__btn is-active" data-oss-filter="all"><?php esc_html_e( 'All', 'astra-child' ); ?></button>
				<?php foreach ( oss_events_types() as $type ) : ?>
					<button type="button" class="oss2-filters__btn" data-oss-filter="<?php echo esc_attr( $type ); ?>"><?php echo esc_html( $type ); ?></button>
				<?php endforeach; ?>
			</div>
			<ol class="oss2-timeline" id="oss2-events-list">
				<?php foreach ( $oss_events as $ev ) :
					...
					?>
					<li class="oss2-timeline__item" data-oss-type="<?php echo esc_attr( ! empty( $ev['type'] ) ? $ev['type'] : '' ); ?>">
			...
			<p class="oss-empty-state oss2-timeline__empty" hidden style="text-align:center;"><?php esc_html_e( 'No events of this type are scheduled right now.', 'astra-child' ); ?></p>
		<?php endif; ?>
	</div>
</section>
